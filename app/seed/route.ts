...
async function seedUsers(sqlInstance = sql) {
  ...
  // Process sequentially to avoid prepared statement conflicts
  const insertedUsers = [];
...
export async function GET() {
  try {
    // Run everything in a transaction, but sequentially
    await sql.begin(async (sqlTransaction) => {
      ...
      await seedRevenue(sqlTransaction);
    });
...
// const sql = postgres(process.env.POSTGRES_URL!, { ssl: 'require' });
// // const sql = postgres(process.env.POSTGRES_URL + '?pgbouncer=true&connection_limit=1', { ssl: 'require' });
